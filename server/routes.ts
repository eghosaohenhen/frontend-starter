...
class Routes {
  ...
  async createPost(session: WebSessionDoc, content_id: ObjectId, postType: PostType, flair: FlairType, caption?: string) {
    if (!Object.keys(FlairType).includes(flair.toString())) {
      throw new InvalidPostFlairError(flair.toString(), Object.keys(FlairType));
    }
    if (!Object.keys(PostType).includes(postType.toString())) {
      throw new InvalidPostFlairError(postType.toString(), Object.keys(PostType));
    }
    const user = WebSession.getUser(session);

    const created = caption ? await Post.create(user, content_id, postType, flair, caption) : await Post.create(user, content_id, postType, flair);
    return { msg: created.msg, post: created.post };
  }
  @Router.get("/posts")
  async getPosts(author?: string, flair?: FlairType, _id?: ObjectId, postType?: PostType) {
    let posts;
    if (author) {
      ...
      posts = await Post.getPosts({ postType: postType });
    } else if (_id) {
